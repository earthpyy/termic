...
`prefs_save` takes a PARTIAL patch and merges it into `settings.json`'s `prefs`
key by key — it never replaces the object, so a key this build doesn't know
(an older or newer profile, a machine that synced the file in) survives. The
frontend debounces and sends only what changed; see the Preferences section of
[data-model.md](data-model.md). `settings_save` is the odd one out: it discards
the `prefs` field on the object it is handed and re-reads disk's copy, because
the Settings UI saves a whole `Settings` snapshot loaded before the pref moved.
